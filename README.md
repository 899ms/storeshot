...
A pre-built Next.js + ShadCN editor for generating iPhone App Store screenshots. Scaffolded by the `app-store-screenshots` skill.
...
Click **Export bundle** to download a zip of 6.9" (1320 × 2868) PNGs. In Connected mode, each PNG is clipped from the connected canvas, so an element that straddles two screens appears split exactly where you placed it. In Isolated mode, each screen clips its own elements and legacy offscreen content cannot leak into neighboring exports.
